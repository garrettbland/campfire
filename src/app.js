...
const Section = ({ block, children }) => {
    ...
    return (
        ...
        >
            ...
            {children}
            <div
                className={`absolute bottom-0 left-0 w-full h-0 bg-orange-500 flex items-center justify-center ${showTool ? "block" : "hidden"
                    }`}
            >
...
const Text = ({ block }) => {
    ...
    return (
        ...
        >
            <button
                onClick={() => editText()}
                className={`absolute top-0 left-0 text-black bg-white p-1 h-full bg-opacity-25 text-lg w-full ${showTool ? "block" : "hidden"
                    }`}
            >
                Edit | Add
      </button>
...
const Image = ({ block }) => {
    ...
    return (
        <div ref={imageRef} className="relative z-10">
            <button
                onClick={() => console.log(returnFound(blocks, { id: block.id }))}
                className={`w-full h-full bg-blue-400 bg-opacity-50 absolute top-0 left-0 z-20 ${showTool ? "block" : "hidden"
                    }`}
            >
                Edit | Add
      </button>
